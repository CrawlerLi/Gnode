...
本项目定位为后端工程实践项目，重点关注区块链节点的核心数据流、模块边界、网络通信和跨节点一致性问题，不是生产级区块链实现。
...
更详细的层级边界说明见 [docs/layer-boundaries.md](docs/layer-boundaries.md)。
...
项目早期使用 Go `gob` 编码作为交易哈希输入。在两节点同步测试中发现，gob 编码会包含类型元数据和类型编号信息，导致不同节点在交易字段一致的情况下仍可能计算出不同的哈希，从而造成 ECDSA 验签失败。

当前交易哈希已改为确定性手写二进制编码，明确规定字段顺序、长度前缀和大端序整数编码，保证跨节点交易哈希和签名验证结果一致。
...
```bash
go test ./internal/core ./internal/service ./internal/wallet ./internal/p2p ./internal/node
```

部分 P2P 测试需要本地 socket 权限，不同运行环境下可能受到限制。

## 当前限制

- 链同步策略仍较基础，目前主要按 Peer 高度拉取缺失区块
- 尚未完整实现分叉选择和链重组
- 尚未实现交易内存池，当前转账会直接打包进入新区块
- 节点运行后的本地控制能力仍依赖 CLI 进程，尚未实现独立 control RPC
- 暂未提供 Docker Compose 和 CI 流程
- 本项目不是生产级区块链实现

## 后续计划

- 增加 Docker Compose，支持一键启动多节点测试环境
- 补充核心单元测试和节点集成测试
- 完善链同步策略、分叉处理和链重组逻辑
- 增加交易内存池和交易广播能力
- 增加 control RPC，支持节点运行期间执行钱包和链操作
- 清理调试输出，引入结构化日志
- 增加 CI 流程
